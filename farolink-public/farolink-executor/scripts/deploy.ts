...
/**
 * FaroLink Production Deployment Script
 *
 * Security checklist enforced by this script:
 *  ✅ Deploys FaroLinkRouter + FeeCollector
...
async function main() {
    ...
    // ── 2. Deploy FaroLinkRouter ─────────────────────────────────────────────
    console.log("\nDeploying FaroLinkRouter...");
    const Router = await ethers.getContractFactory("FaroLinkRouter");
    const router = await Router.deploy(feeCollectorAddress);
    await router.waitForDeployment();
    const routerAddress = await router.getAddress();
    console.log("✅ FaroLinkRouter deployed:", routerAddress);
    ...
    console.log("══════════════════════════════════════════════════════");
    console.log("FaroLinkRouter: ", routerAddress);
