...
/**
 * All errors thrown by PharosflowClient are instances of PharosflowError.
 * This lets consumers catch and branch on SDK errors cleanly:
 *
 *   try { await client.getQuote(...) }
 *   catch (e) {
 *     if (e instanceof PharosflowError) { console.log(e.statusCode, e.code) }
 *   }
 */
export class PharosflowError extends Error {
    constructor(
        message: string,
        ...
        super(message);
        this.name = 'PharosflowError';
    }
}

/** Converts an axios error into a structured PharosflowError */
function wrapAxiosError(err: unknown): never {
    if (axios.isAxiosError(err)) {
        ...
        throw new PharosflowError(message, status, code, body);
    }
...
/**
 * EIP-712 domain and type definitions for PharosFlow intent signing.
 * Exported so advanced users can build custom signing flows.
 * Must stay in sync with pharosflow-api/src/index.ts INTENT_DOMAIN / INTENT_TYPES.
 */
export const PHAROSFLOW_INTENT_DOMAIN = {
    name:    'PharosFlow',
    version: '1',
} as const;

export const PHAROSFLOW_INTENT_TYPES = {
    BridgingIntent: [
...
function validateRouteRequest(req: RouteRequest): void {
    if (!req.fromChain || !req.toChain)
        throw new PharosflowError('RouteRequest: fromChain and toChain are required', undefined, 'INVALID_INPUT');
    if (!EVM_ADDRESS_RE.test(req.fromToken))
        throw new PharosflowError(`RouteRequest: fromToken "${req.fromToken}" is not a valid EVM address`, undefined, 'INVALID_INPUT');
    if (!EVM_ADDRESS_RE.test(req.toToken))
        throw new PharosflowError(`RouteRequest: toToken "${req.toToken}" is not a valid EVM address`, undefined, 'INVALID_INPUT');
    if (!WEI_STRING_RE.test(req.amountIn) || BigInt(req.amountIn) === 0n)
        throw new PharosflowError(`RouteRequest: amountIn "${req.amountIn}" must be a non-zero wei amount as a decimal string`, undefined, 'INVALID_INPUT');
    if (req.userAddress && !EVM_ADDRESS_RE.test(req.userAddress))
        throw new PharosflowError(`RouteRequest: userAddress "${req.userAddress}" is not a valid EVM address`, undefined, 'INVALID_INPUT');
}

function validateIntent(intent: BridgingIntent): void {
    if (!EVM_ADDRESS_RE.test(intent.sourceUserAddress))
        throw new PharosflowError('BridgingIntent: sourceUserAddress is not a valid EVM address', undefined, 'INVALID_INPUT');
    if (!intent.sourceChainId || !intent.targetChainId)
        throw new PharosflowError('BridgingIntent: sourceChainId and targetChainId are required', undefined, 'INVALID_INPUT');
    if (!WEI_STRING_RE.test(intent.amountIn))
        throw new PharosflowError('BridgingIntent: amountIn must be a decimal string', undefined, 'INVALID_INPUT');
    if (intent.deadline < Math.floor(Date.now() / 1000))
        throw new PharosflowError('BridgingIntent: deadline has already passed', undefined, 'INTENT_EXPIRED');
}

// ─── Client ───────────────────────────────────────────────────────────────────

export class PharosflowClient {
    private client: AxiosInstance;

    constructor(config: ClientConfig = {}) {
        this.client = axios.create({
            baseURL: config.apiUrl ?? 'https://api.pharosflow.net',
            // Default 30s timeout — prevents callers from hanging indefinitely
            timeout: config.timeoutMs ?? 30_000,
            headers: {
                'Content-Type': 'application/json',
                ...(config.apiKey && { 'x-pharosflow-api-key': config.apiKey }),
            },
        });
    }
     ...
     * @throws          PharosflowError on invalid input or API error
     *
     * @example
     ...
     * @throws        PharosflowError if the intent is already expired or invalid
     *
     * @example
    ...
    async signIntent(
        ...
        const signature = await signer.signTypedData(domain, PHAROSFLOW_INTENT_TYPES, value);
     ...
     * @throws        PharosflowError if signature is missing or the API rejects the intent
     */
    async executeIntent(intent: BridgingIntent): Promise<ExecutionResponse> {
        if (!intent.signature) {
            throw new PharosflowError(
     ...
     * @throws              PharosflowError on API error
     */
    async getStatus(trackingHash: string): Promise<StatusResponse> {
        if (!trackingHash) {
            throw new PharosflowError('trackingHash is required', undefined, 'INVALID_INPUT');
        }
        try {
     ...
     * @throws                     PharosflowError with code 'INTENT_FAILED' or 'TRACKING_TIMEOUT'
     *
     * @example
     * const result = await client.executeIntent(signed);
