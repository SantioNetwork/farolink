...
export function assertTokenAddressesAreReal(): void {
  // Frontend-only assertion: import.meta is not supported in the backend Node.js (CommonJS) environment.
  // We skip this check on the backend.
}

export const TOKENS: TokenInfo[] = [

  // ─── Pharos Testnet (chainId 688689) — Real deployed contract addresses ─────
...
export async function fetchLivePrices(): Promise<Record<string,number>> {
  ...
  try {
    ...
    for (const [sym,id] of Object.entries(COINGECKO_IDS)) next[sym] = data[id]?.usd ?? priceCache[sym] ?? 0;
    priceCache = next; priceCacheTime = Date.now(); return next;
  } catch {
    return Object.keys(priceCache).length ? priceCache : {
