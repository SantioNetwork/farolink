...
app.use(cors({
    ...
    methods: ['GET', 'POST'],
    allowedHeaders: ['Content-Type', 'x-farolink-api-key'],
}));
...
const requireApiKey = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
    const rawKey = req.headers['x-farolink-api-key'] as string;
...
async function main() {
    const server = app.listen(env.PORT, () => {
        logger.info(`FaroLink API Gateway on port ${env.PORT} [${env.NODE_ENV}]`);
    });
