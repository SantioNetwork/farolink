 ...
 *     the HMAC, and send it as the X-FaroLink-Signature header
            ...
            try {
                const res = await axios.post(webhook.url, body, {
                    headers: {
                        ...
                        'X-FaroLink-Signature': `sha256=${signature}`,
                        'X-FaroLink-Event':     payload.event,
                        'X-FaroLink-Timestamp': String(payload.timestamp),
                    },
                    timeout: 5000,
                });
